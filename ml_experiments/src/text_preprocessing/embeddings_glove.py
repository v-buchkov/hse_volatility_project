...
def get_vectors_gt100(row, model, word_doc_freq, vocabulary):
    vecs = [np.zeros(100)]
    for word in row:
        try:
            # если слово есть в нашем очищенном словаре
            # умножаем вектор на вес tfidf
            v = model[word] * word_doc_freq[vocabulary[word]]
        except:
            v = np.zeros(100)
        vecs.append(v)
...
def glove_embedding(texts_df: pd.DataFrame, model_used: str = 'glove-twitter-100', min_count: int = 5,
    ...
    model_t = api.load(model_used)

    gt100 = np.array([get_vectors_gt100(i, model_t, word_doc_freq, vocabulary) for i in train_tokenized])
    return gt100
